...
OUTPUT_PATH = "./data"
TXT_STR = ".*/-+--='`;./\ ,,,,,.......????????????的一是不了在有人这上大来和我个中地为他生要们以到国时就出说会也子学发着对作能可于成用过动主下而年分得家种里多经自现同后产方工行面那小所起去之都然理进体还定实如么物法你好性民从天化等力本长心把部义样事看业当因高十开些社前又它水其没想意三只重点与使但度由道全制明相两情外间二关活正合者形应头无量表象气文展"
TXT_STR += "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
TXT_STR += "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
...
for i in range(9600):
    ...
    # 定义字体和大小
    font_path = random.sample(["ttf/microsoftYaHei.ttf", "ttf/NotoSansSC-VariableFont_wght.ttf", "ttf/NotoSansSC-SemiBold.ttf", "ttf/NotoSansSC-Black.ttf", "ttf/NotoSansSC-Thin.ttf", "ttf/NotoSansSC-Thin.ttf"], 1)[0]  # 这里需要指定一个中文字体的路径
    font_size = random.sample([random.randrange(8,32), random.randrange(8,32), random.randrange(8,32), random.randrange(32,48)],1)[0]
...
for (index,ground) in grounds:
    ...
    cv2.imwrite(f'{OUTPUT_PATH}/data_lr_{index}.jpg', downsampled_image, [int(cv2.IMWRITE_JPEG_QUALITY),95])
