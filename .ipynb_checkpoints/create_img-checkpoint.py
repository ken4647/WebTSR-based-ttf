...
for i in range(100):
    ...
    # 定义文字颜色为白色
    text_attr["text_color"] = random.randrange(0,224)
    # 定义文字的放置位置
    text_attr["position"] = (random.randrange(0,height), random.randrange(0,width))
...
for (index,ground) in grounds:
    ...
    downsampled_image = cv2.resize(image, None, fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)
    cv2.imwrite(f'{OUTPUT_PATH}/data_lr_{index}.jpg', downsampled_image)
